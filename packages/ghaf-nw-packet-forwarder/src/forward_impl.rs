...
pub mod forward {
    ...
    pub fn is_iface_running_up(iface_name: &str) -> bool {
        ...
        // Find the interface by name and check its properties
        for iface in interfaces.iter() {
            if iface.name == iface_name
                ...
                && iface.is_running()
            {
                // Iterate over assigned IPs to find an IPv4 address
                for ip in &iface.ips {
                    if let IpNetwork::V4(ipv4) = ip {
                        let current_ifaces = get_ifaces();
                        if current_ifaces.ext_mac == iface.mac.unwrap()
                            && current_ifaces.ext_ip.ip() != ipv4.ip()
                        {
                            let mut ifaces = IFACES.write().unwrap();
                            ifaces.ext_ip = IpNetwork::V4(*ipv4);
    ...
    pub async fn set_sec_params(rate_limiter_enabled: bool, cancel_token: CancellationToken) {
        let security = Arc::clone(&SECURITY);
        security
            .clone()
            .set_rate_limiter(rate_limiter_enabled)
            .await;
    ...
    async fn ext_to_int_is_packet_safe(eth_packet: &mut MutableEthernetPacket<'_>) -> bool {
        ...
        if total_packet_len <= MIN_PACKET_SIZE || total_packet_len >= MAX_PACKET_SIZE {
