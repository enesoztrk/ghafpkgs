...
GDBusNodeInfo* introspect_node(GDBusConnection* conn, const gchar* bus_name,
    ...
    reply = g_dbus_connection_call_sync(conn, bus_name, object_path, DBUS_INTERFACE_INTROSPECTABLE,
...
gboolean proxy_object_manager_objects(gchar* object_manager_path) {
    ...
    GVariant* result = g_dbus_connection_call_sync(
        proxy_state->source_bus, proxy_state->config.source_bus_name, object_manager_path,
    ...
    Log::info() << "=== ObjectManager Managed Objects ===";
    ...
    while (g_variant_iter_next(objects_iter, "{&o@a{sa{sv}}}", &object_path, &interfaces_var)) {
        Log::info() << "Object: " << object_path;

        GDBusNodeInfo* node_info = introspect_node(
...
gboolean discover_and_proxy_object_tree(const gchar* base_path, gchar** foundObjectManagerPath,
                                        gboolean need_lock) {
    GError* error = nullptr;
    ...
    if (node_info->interfaces) {
        for (int i = 0; node_info->interfaces[i]; i++) {
            ...
            if (foundObjectManagerPath && g_strcmp0(node_info->interfaces[i]->name,
                                                    "org.freedesktop.DBus.ObjectManager") == 0) {
                *foundObjectManagerPath = g_strdup(base_path);
                Log::info() << "ObjectManager found at: " << *foundObjectManagerPath;
...
gboolean register_single_interface(const gchar* object_path, const gchar* interface_name,
...
gboolean setup_proxy_interfaces() {
    ...
    if (object_manager_path) {
        Log::info() << "ObjectManager interface proxied at: " << object_manager_path;
...
void on_name_acquired_log(G_GNUC_UNUSED GDBusConnection* conn, const gchar* name,
                          gpointer user_data G_GNUC_UNUSED) {
...
void validateProxyConfigOrExit(ProxyConfig* config) {
    ...
    if (config->sni_mode && config->renamer_mode) {
        Log::error() << "Error: --sni-mode and --renamer-mode cannot be used together.";
        exit(EXIT_FAILURE);
    }
